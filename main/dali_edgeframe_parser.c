...
#define DALI_TIMING_TOLERANCE_US 280
...
int get_frame_type_from_duration(uint16_t duration)
{
    ...
    if ((duration < DALI_BACKWARD_FRAME_DURATION + DALI_FRAME_DURATION_TOLERANCE) && (duration > DALI_BACKWARD_FRAME_DURATION - DALI_FRAME_DURATION_TOLERANCE))
    {
...
void edgeframe_queue_log_task(void *params)
{
    ...
    while (1)
    {
        ...
        default:
            ...
            frameaction = pass->config.mangled_frame_action;
        }
        log = frameaction & 1;
        ...
        if (received && log)
        {
            ...
            for (uint8_t i = 0; i < receivedframe.length; i++)
            {
                ...
                if (error || strobestate == EDGEDECODE_STATE_END)
                    break;
                ...
                case EDGEDECODE_STATE_BITREADY:
                    ...
                    {
                        if (debug)
                            ESP_LOGD(TAG, "...Half bit %u", i);
                        // do nothing
                    }
                    else if (check_if_full_period(last_valid_bit_elapsed))
                    {
                        if (debug)
                            ESP_LOGD(TAG, "...Full bit %u edge %d bitpos %d", i, edge.edgetype, output_bit_pos);
                        if (edge.edgetype == EDGETYPE_RISING)
                        {
            ...
            if (error)
            {
                outputframe.type = DALI_MANGLED_FRAME;
                frameaction = pass->config.mangled_frame_action;
                log = frameaction & 1;
                record = frameaction & 2;
            }
            ...
            if (record)
            {
                ...
                if (sendsuccess != pdTRUE)
                    ESP_LOGE(TAG, "Dali output queue full");
            }
            if (log)
            {
                ...
                if (error)
                {
                    for (uint8_t i = 0; i < receivedframe.length; i++)
                    {
                        edge_t edge = receivedframe.edges[i];
                        ESP_LOGI(TAG, "Edge %s: %i (%i)", edge.edgetype ? "+" : "-", (int)edge.time - timelast, edge.time);
                        timelast = (int)edge.time;
                    }
                }
            }
        }
