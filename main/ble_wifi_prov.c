...
const int WIFI_RESTART_PROV_BIT = BIT2;
...
{
    static int retries;
    networking_ctx_t* ctx = (networking_ctx_t*) arg;
    if (event_base == WIFI_PROV_EVENT) {
        switch (event_id) {
            ...
            case WIFI_PROV_END:
                ...
                // unblock main task
                xTaskNotifyIndexed(ctx->mainloop_task, PROVISIONING_DONE_INDEX, 0, eNoAction);
                break;
            default:
                break;
        }
    ...
    else if (event_base == WIFI_EVENT)
    {
        ...
        else if (event_id == WIFI_EVENT_STA_DISCONNECTED)
        {
            ...
            ESP_LOGI(TAG, "WIFI_EVENT_STA_DISCONNECT received - reason %d", event->reason);
...
static void wifi_setup_main_operation_events(void)
{
...
void wifi_provision(networking_ctx_t* ctx)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_PROV_EVENT, ESP_EVENT_ANY_ID, &event_handler, ctx));
// #ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
    ESP_ERROR_CHECK(esp_event_handler_register(PROTOCOMM_TRANSPORT_BLE_EVENT, ESP_EVENT_ANY_ID, &event_handler, ctx));
// #endif
    ESP_ERROR_CHECK(esp_event_handler_register(PROTOCOMM_SECURITY_SESSION_EVENT, ESP_EVENT_ANY_ID, &event_handler, ctx));
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &event_handler, ctx));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &event_handler, ctx));
...
// #endif /* CONFIG_EXAMPLE_PROV_TRANSPORT_BLE */
        ...
        * so let's release it's resources */
        ...
        /* Start Wi-Fi station */
        wifi_setup_main_operation_events();
    }
