...
void app_main(void) {
    ...
    int dali_levels_to_send[DALI_CHANNELS] = {0};
    ...
    while (1) {
        ...
        //         ESP_LOGI(TAG, "Lvl-> SP PWR | SRCE N | 0-10v1 0-10v2 DALIA
        //         DALIB DALIC DALID DALIE DALIF ESPN Rly1 Rly2    Fade    TLT
        //         LT Wake");
        //     }
        //     ESP_LOGI(TAG, "%3.1u->%3.1d %s | %s %1.i |    %s    %s   %s   %s
        //     %s   %s   %s   %s  %s  %s  %s %7.1i %6.1lu %4.1i  %3.1lu",
        //              status.actual_level,
        //              status.setpoint,
        //              (status.power_on) ? " ON" : "OFF",
        //              source_str[status.setpoint_source & 0xF],
        //              (int)new_setpoint,
        //              (configbits & CONFIGBIT_USE_0_10v1) ? zeroten1_str :
        //              spaces, (configbits & CONFIGBIT_USE_0_10v2) ?
        //              zeroten2_str : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[0]
        //              != -1) ? dali_str[0] : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[1]
        //              != -1) ? dali_str[1] : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[2]
        //              != -1) ? dali_str[2] : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[3]
        //              != -1) ? dali_str[3] : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[4]
        //              != -1) ? dali_str[4] : spaces,
        //              ((configbits & CONFIGBIT_USE_DALI) && dali_addresses[5]
        //              != -1) ? dali_str[5] : spaces, (configbits &
        //              CONFIGBIT_TRANSMIT_ESPNOW) ? espnow_str : spaces,
        //              (configbits & CONFIGBIT_USE_RELAY1) ? relay1_str :
        //              spaces, (configbits & CONFIGBIT_USE_RELAY2) ? relay2_str
        //              : spaces, fadetime, tlt_array[status.actual_level],
        //              ((int)actual_looptime),
        //              idle_reawake_interval >> 10);
        // }
        // else
        ...
        //         // log legend every 8 logs
        //         ESP_LOGI(TAG, "Lvl-> SP PWR | SRCE N | 0-10v1 0-10v2 DALIA
        //         DALIB DALIC DALID DALIE DALIF ESPN     Fade    TLT   LT
        //         Wake");
        //     }
        //     ESP_LOGI(TAG, "%3.1u->%3.1d %s | %s %1.i |    %s    %s   %s   %s
        //     %s   %s   %s   %s  %s  %7.1i %6.1lu %4.1i  %3.1lu",
        //              status.actual_level,
        //              status.setpoint,
        //              (status.power_on) ? " ON" : "OFF",
        //              source_str[status.setpoint_source & 0xF],
